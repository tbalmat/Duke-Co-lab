...
readData <- function() {
  read.table("Data/GWASResults.csv", header=T, sep=",", strip.white=T)
}
...
assembleNetComponents <- function() {
  ...
  gwas <- merge(gwas1[which(gwasdat[,"GWAS"]==1 & gwasdat[,"log_10_p"]>=log_10_p),], 
                gwas2[which(gwasdat[,"GWAS"]==2 & gwasdat[,"log_10_p"]>=log_10_p),], by="SNP", suffixes=1:2)
...
shinyServer(
  function(input, output, session) {
    ...
    observe({
      ...
      net <- assembleNetComponents(gwas, log_10_p, physics, eopacity)
