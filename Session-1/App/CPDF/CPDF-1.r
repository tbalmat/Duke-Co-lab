...
if(is.null(facetVar)) {
  ...
  gdat <- aggregate(cpdf[,"n"], by=list(cpdf[,indepVar], cpdf[,facetVar]), sum)
  colnames(gdat) <- c(indepVar, facetVar, "n")
  ng <- aggregate(gdat[,"n"], by=list(gdat[,facetVar]), sum)
  colnames(ng) <- c(facetVar, "n")
  gdat <- merge(gdat, ng, by.x=facetVar, by.y=facetVar)
  gdat <- data.frame(gdat[,c(facetVar, indepVar, "n.x")], "p"=gdat[,"n.x"]/gdat[,"n.y"])
  colnames(gdat) <- c(facetVar, indepVar, "n", "p") 
}
...
g <- g + geom_smooth(data=gdat, aes_string(x=indepVar, y="p"),
                     method="loess", se=F, span=loessSpan, fullrange=T, color="Black", size=.6)
