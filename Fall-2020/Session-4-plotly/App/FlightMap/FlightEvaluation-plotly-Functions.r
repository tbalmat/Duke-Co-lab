...
aggfdat01 <- function(data, aggVar, apdat) {
  ...
  if(nrow(data)>0) {
    ...
    if(aggVar!="") {
      fd <- do.call(rbind,
                    apply(as.matrix(unique(data[,aggVar])), 1,
                      function(x) {
                        k <- which(data[,aggVar]==x)
                        fd <- aggregate(k, by=list(data[k,"OriginAirportID"], data[k,"DestAirportID"]), length)
                        fd <- data.frame(x, fd[,1:2], fd[,3]/length(k))
                        return(setNames(fd, c(aggVar, "OriginAirportID", "DestAirportID", "p")))
                      }))
    } else {
      fd <- aggregate(1:nrow(data), by=list(data[,"OriginAirportID"], data[,"DestAirportID"]), length)
      fd[,3] <- fd[,3]/nrow(data)
    ...
    fd <- data.frame("OriginAirportID"=character(), "DestAirportID"=numeric(), numeric(), "p"=numeric())
...
print(fldat)
  ...
  g <- ggplot() +
       ...
       geom_curve(data=fldat,
                  aes(x=longitudeOrigin, xend=longitudeDest, y=latitudeOrigin, yend=latitudeDest,
                      color=p, size=p, alpha=p),
       ...
       scale_color_gradient2(name="proportion flights",
                             low=colorRange[1], mid=colorRange[2], high=colorRange[3], midpoint=colorScaleMid) +
