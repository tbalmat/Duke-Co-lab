...
shinyServer(
  ...
  function(input, output, session) {
      ...
      input$t1ActionComposeTable, {
        ...
        if(length(input$t1DepVar)>0 & length(input$t1IndepVar)>0) {
          ...
          iagg <<- aggregate(1:nrow(cpdf),
                             by=lapply(input$t1IndepVar,
                                       function(v)
                                         if(v=="Agency" & input$t1AgencyPos<agencyPosLimit[2]) {
                                           substring(cpdf[,v], 1, input$t1AgencyPos)
                                         } else if(v=="Occupation" & input$t1OccPos<occPosLimit[2]) {
                                           substring(cpdf[,v], 1, input$t1OccPos)
                                         } else {
                                           cpdf[,v]
                                         }),
                             function(k) k)
          colnames(iagg) <<- c(input$t1IndepVar, "k")
          ...
          if(is.data.frame(iagg)) {
            ...
            aggdat <- data.frame(iagg[,input$t1IndepVar],
                        ...
                        do.call(rbind,
                          apply(as.matrix(1:nrow(iagg)), 1,
                            function(i) {
                              k <- iagg[i,"k"][[1]]
                              data.frame(length(k),
                                         mean(cpdf[k,input$t1DepVar]),
                                         # Use transpose, otherwise n and mean are recycled for each quantile
                                         t(quantile(cpdf[k,input$t1DepVar], c(0.25, 0.5, 0.75))))
                            })))
    ...
    #######################################################################################################
    # Function to compose box plot for subset of observations corresponding to specified table row
    #######################################################################################################
