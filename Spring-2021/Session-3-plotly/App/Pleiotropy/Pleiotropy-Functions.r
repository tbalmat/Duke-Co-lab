...
t1ComposePlot1 <- function(data, ylim, xyHighlight=NULL) {
  ...
  # All points in blue
  g <- g + geom_jitter(aes(x=phenotype, SNP=SNP, Ph=phenotype, y=log_10_p), width=0.1, color="blue3", alpha=0.5)
...
t2ComposePlot <- function(x1, y1, x2, y2, vertex1, vertex2, emphset1, emphset2, selset,
  ...
  g <- plot_ly(
         # Vertex1 (phenotype from GWAS set 1) labels on left
         x=x1-0.1, y=y1,
       ...
       # Add points preceding above text
