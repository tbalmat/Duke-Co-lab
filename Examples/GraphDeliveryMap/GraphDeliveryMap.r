...
# Specify number of carriers and customer locations, Earth radius (miles),
...
erad <- 3959
srvlat <- c(35.6, 40)[1]
srvlong <- c(-79.8, -100)[1]
srvrad <- c(100, 500, 750)[1]
pi <- 4*atan(1)
degtorad <- pi/180
...
# while it varyies longitudinally depending on the latitude of a location
...
setwd("C:\\Projects\\R\\GraphDB\\GraphTraversal")
...
#kfacility <- which(loc[,"City"]=="ASHEBORO")
...
ggplot() +
  ...
  geom_point(data=loc[kcustomer,], aes(x=LongDeg, y=LatDeg, color=as.character(cclust[["cluster"]]),
             shape="customer"), size=2, alpha=0.5) +
  geom_point(data=data.frame(cclust[["centers"]]/degtorad), aes(x=Long, y=Lat, fill=as.character(1:ncarr),
             shape="zone center"), size=2, alpha=0.5) +
  geom_point(data=data.frame(cclust[["centers"]]/degtorad), aes(x=Long, y=Lat), shape=21, size=3, alpha=0.5) +
  geom_point(data=loc[kcarrier,], aes(x=LongDeg, y=LatDeg, color=as.character(carclust),
             shape="carrier"), size=2.5, alpha=1) +
  geom_point(data=loc[kcarrier,], aes(x=LongDeg, y=LatDeg), shape=21, size=4, color="black") +
  geom_point(data=loc[kfacility,], aes(x=LongDeg, y=LatDeg, shape="facility"), size=2.5) +
  geom_point(data=loc[kfacility,], aes(x=LongDeg, y=LatDeg), shape=5, size=4) +
